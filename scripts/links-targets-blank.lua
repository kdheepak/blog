...
local function add_target_blank (link)
    if string.match(link.target, "^http") ~= true and ( string.match(link.target, "blog.kdheepak.com") == nil or string.match(link.target, "blog.kdheepak.com") == false ) then  -- here .target == href attribute
